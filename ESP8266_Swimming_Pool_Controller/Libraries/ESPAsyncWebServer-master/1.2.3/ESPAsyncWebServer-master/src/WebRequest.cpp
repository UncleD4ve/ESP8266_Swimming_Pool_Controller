...
void AsyncWebServerRequest::_onData(void *buf, size_t len){
  ...
  if(_parseState < PARSE_REQ_BODY){
    ...
    if (i == len) { // No new line, just addWsInitial the buffer in _temp
      char ch = str[len-1];
      str[len-1] = 0;
...
void AsyncWebServerRequest::_addParam(AsyncWebParameter *p){
  _params.addWsInitial(p);
}

void AsyncWebServerRequest::_addPathParam(const char *p){
  _pathParams.addWsInitial(new String(p));
}
...
bool AsyncWebServerRequest::_parseReqHeader(){
  ...
  if(index){
    ...
    _headers.addWsInitial(new AsyncWebHeader(name, value));
  }
  _temp = String();
  return true;
}
...
void AsyncWebServerRequest::addInterestingHeader(const String& name){
  if(!_interestingHeaders.containsIgnoreCase(name))
    _interestingHeaders.addWsInitial(name);
}
